...
const REPORTS_MAP = {
  ...
  'SAE15': 'Thales05_SAE15_Rapport (6).pdf',
  'PHOTO_ATB': 'Thales05_SAE15_Rapport (6).pdf',

  // Projets ajoutés / complétés
  'SAE31': 'SAE 31-Transmissions.pdf',
  'R315': 'site_with_reports_no_section_btn/JUILLET_Etienne_plan_management.pdf',
  'SAE32': 'site_with_reports_no_section_btn/SAE32_1A_JUILLET_RapportIA.pdf',
  'PENTEST': 'site_with_reports_no_section_btn/RAPPORT_PENTEST_JUILLET_Etienne-1.pdf',

  // Libellés spécifiques
  'KATO': { path: 'site_with_reports_no_section_btn/packet.pkt', label: 'TÉLÉCHARGER LE PACKET TRACER' }
};
/* =========================
...
const secIds = ['accueil','apropos','projets','competences','contact'];
...
if(!document.body.dataset.current){ setCurrentSection(secIds[0]); }
...
btnDownOne?.addEventListener('click', ()=>{
  const current = document.body.dataset.current;
  const idx = secIds.indexOf(current);
  if(idx >= 0 && idx < secIds.length - 1){
    smoothScrollTo(document.getElementById(secIds[idx+1]));
  }
});
...
function openModalFrom(card){
  ...
  const explicitKey = (card.dataset.report || '').trim();
  const titleText = (card.querySelector('h3')?.textContent || '').trim();
  const key = (explicitKey || titleText).toUpperCase();
  if (modalActions){
    modalActions.innerHTML = '';
    const entry = REPORTS_MAP[key];
    if(entry){
      const a = document.createElement('a');
      a.className = 'cv';

      const path = (typeof entry === 'string') ? entry : entry.path;
      const label = (typeof entry === 'string') ? 'TÉLÉCHARGER LE RAPPORT' : (entry.label || 'TÉLÉCHARGER LE RAPPORT');
  ...
  const objective = (card.dataset.objective || '').trim();
  const details = (card.dataset.long || '').trim();
  modalDesc.innerHTML = [objective, details]
    .filter(Boolean)
